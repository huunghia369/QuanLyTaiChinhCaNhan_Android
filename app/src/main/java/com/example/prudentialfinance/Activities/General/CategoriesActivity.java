...
public class CategoriesActivity extends AppCompatActivity {
    ...
    private boolean isExpenseFragmentVisible = false; // Mặc định là không hiển thị fragment chi tiêu
    ...
    private void setEvent() {
        ...
        // Chuyển sang trang Transassaction
//        backBtn.setOnClickListener(view -> {
////                chuyển từ trang login sang trang the loai
//            Intent intent = new Intent(CategoriesActivity.this, TransactionActivity.class);
...
//        });
        ...
        // GPT ẩn btnAdd Chi tiêu
        viewPager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
            @Override
            public void onPageSelected(int position) {
                if (position == 1) { // Kiểm tra khi chuyển đến fragment chi tiêu
                    isExpenseFragmentVisible = true;
                    btnAdd.setVisibility(View.GONE); // Ẩn button btnAdd
                } else {
                    isExpenseFragmentVisible = false;
                    btnAdd.setVisibility(View.VISIBLE); // Hiện button btnAdd
                }
            }
        });
